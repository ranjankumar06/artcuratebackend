...
router.put("/addlinkedUserName/:id", (req, res) => {
  let id = req.params.id;
  if (!userMiddleware.validateUserName(req.body.username))
    return res.status(400).json({ message: "Invalid username." });
...
router.get(
  ...
  async (req, res) => {
    User.LinkedUsers.find(
      { linkWith: ObjectId(req.params.id) },
      async (err, data) => {
        if (err) {
          res.send(err.message);
        } else {
...
router.post("/sendSms", (req, res) => {
  ...
  params.append("message", `Hi ${name}, Welcome to Artcurate`);
  tlClient
    .post("/send", params)
    .then((resp) => {
        ...
        User.Auth.findOne({ email }).exec(async (err, user) => {
          if (err) {
            ...
            if (user) {
              ...
              const token = jwt.sign({ _id: user._id }, config.secrateKey, {
                expiresIn: "10d",
              });
              ...
              newUser.save((err, data) => {
                if (err) {
                  ...
                  const token = jwt.sign({ _id: data._id }, config.secrateKey, {
                    expiresIn: "7d",
                  });
...
router.get("/userNameAvailabiity/:id", (req, res) => {
  let id = req.params.id;
  console.log(
    "🚀 ~ file: controller.js ~ line 960 ~ User.Auth.findOne ~ req.body.username",
    req.body
  );
  ...
  User.Auth.findOne({ username: req.body.username }, (err, data) => {
    if (err) {
      ...
      if (data) {
        console.log(
          "🚀 ~ file: controller.js ~ line 940 ~ User.Auth.findOne ~ data",
          data
        );
        let errorMsg = "";
        //need to optimize code
    ...
    (err, role) => {
      if (err) {
        ...
        if (role) {
          ...
          if (
            (_.toLower(category) == "artist") |
            (_.toLower(category) == "enthusiast")
          ) {
            User.Auth.findOneAndUpdate(
              { _id: ObjectId(id) },
              {
...
router.get("/getNotifications/:userId", async (req, res) => {
  try {
    ...
    const result = await User.Notification.findOne(
      { userId },
...
router.post("/pauseAll", (req, res) => {
  ...
  if (req.body.isPauseAll != true) {
    ...
    User.Notification.findOneAndUpdate(
      resSettingFilter,
      resSettingData,
      options,
      (error, result) => {
...
router.post("/whatsappNotification", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isWhatsappNotification: req.body.isWhatsappNotification,
  };
  User.Notification.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
...
router.post("/requestReceived", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isRequestReceived: req.body.isRequestReceived,
  };
  User.Notification.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
...
router.post("/requestAccepted", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isRequestAccepted: req.body.isRequestAccepted,
  };
  User.Notification.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
...
router.post("/studentAccountOnly", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isStudentOnly: req.body.isStudentOnly,
  };
  User.Notification.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
...
router.get("/getTagsData/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const result = await User.TagsDataSharing.findOne(
      { userId },
      { _id: 0, userId: 0, createdAt: 0, updatedAt: 0 }
    );
...
router.post("/reviewTags", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isReviewTags: req.body.isReviewTags,
  };

  User.TagsDataSharing.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
    options,
    (error, result) => {
...
router.post("/collabRequest", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isCollaborationRequest: req.body.isCollaborationRequest,
  };
...
router.get("/getPrivacyData/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const result = await User.Privacy.findOne(
      { userId },
      { _id: 0, userId: 0, createdAt: 0, updatedAt: 0 }
    );
...
router.post("/supOrSub", async (req, res) => {
  try {
    ...
    const resSettingData = {
      userId: req.body.userId,
      SupporterOrSubscriber: req.body.SupporterOrSubscriber,
    };
    const result = await User.Privacy.findOne(
      { resSettingFilter },
      { _id: 0, userId: 0, createdAt: 0, updatedAt: 0 }
...
router.post("/supporter", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isSupporter: req.body.isSupporter,
...
router.post("/canMention", async (req, res) => {
  try {
    ...
    const resSettingData = {
      userId: req.body.userId,
      canMention: req.body.canMention,
    };
    const result = await User.Privacy.findOne(
      { resSettingFilter },
      { _id: 0, userId: 0, createdAt: 0, updatedAt: 0 }
...
router.get("/getLocationData/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const result = await User.Location.findOne(
      { userId },
      { _id: 0, userId: 0, createdAt: 0, updatedAt: 0 }
    );
...
router.post("/appLocation", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isUsingApp: req.body.isUsingApp,
  };

  User.Location.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
    options,
    (error, result) => {
...
router.post("/mapLocation", (req, res) => {
  ...
  const resSettingData = {
    userId: req.body.userId,
    isAllowMap: req.body.isAllowMap,
  };
...
router.get("/getAllHistorylogs/:userId", async (req, res) => {
  ...
  try {
    const result = await User.HistoryLogs.findOne(
      { userId },
...
router.post("/addMapLocation", (req, res) => {
  ...
  User.mapLoaction.findOneAndUpdate(
    resSettingFilter,
    resSettingData,
    options,
    (err, result) => {
      ...
      User.Auth.findOne({ _id: user.userId }, (err, data) => {
        if (err) {
          ...
          for (i = 0; i < user.teammeber.length; i++) {
            const url =
              "https://localhost:3000/user/teammemberRequest?id=" + user.id;
            if (user.teammeber[i].email) {
              let = emailid = user.teammeber[i].email;

              email(emailid, "Invite Link", "Invite Member", "teamMember", {
                fullName,
                emailid,
                url,
              });
            ...
            User.Auth.findOne(
              {
                $or: [
                  { email: user.teammeber[i].email },
                  { phone: user.teammeber[i].phone },
                ],
              },
              (err, result) => {
                if (err) {
          ...
          res.status(200).send({
            success: true,
            message: " Notification  is sent to users mobile number",
          });
        }
      });
    }
    ...
    if (err) {
      ...
      User.Auth.findOne({ _id: user.userId }, (err, data) => {
        if (err) {
          ...
          res.status(200).send({
            success: true,
            message: " Invitation  is sent to users",
          });
        }
      });
    }
...
router.post("/AcceptTeamRequset", async (req, res) => {
  ...
  User.AllNotifications.findOneAndUpdate(
    req.body,
    resSettingData,
    options,
    (error, result) => {
