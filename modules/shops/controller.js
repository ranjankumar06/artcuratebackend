            ...
            if (shop) {
                ...
                shopAccountModel.save((err, user) => {
                    if (err) {
                        ...
                        const addressWithId = req.body.address.map(i => ({...i, shopId: shop._id}))
                        Shop.Address.insertMany(addressWithId).then(function () {
                            const bodyText = "Shop created successfully. Your shop will be activated ASAP";
                            email(req.body.email, 'Welcome to Artcurate', bodyText, bodyText).then(data => {
                                res.send(data);
                            }, err => {
                                res.send(err);
                            });
...
router.put("/update/:id", userMiddleware.verifyToken, (req, res) => {
    ...
    Shop.Details.findOneAndUpdate({ _id: id }, req.body, {
...
router.put("/update/accounts/:id", userMiddleware.verifyToken, (req, res) => {
    let id = req.params.id;
    Shop.Accounts.findOneAndUpdate({ _id: id }, req.body, {
        timestamps: { createdAt:false, updatedAt:true }
    }, (err, data) => {
    if (err) {
                            console.log(error)      // Failure
